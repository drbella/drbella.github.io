...
BEGIN
{
  ...
  @referers          = qw(drbella.com.au www.drbella.com.au 4tknox.au.com secure.netregistry.net);
  @allow_mail_to     = qw();
  @recipients        = ('^(([^\-][\w\-]*?\.?){1,})[^-](\@{1})drbella.com.au');
  %recipient_alias   = ();
...
# (no user serviceable parts beyond here)
  ...
  # Added by NetRegistry, Lawrence Ong, 20021107 - To prevent spam abuse
  # allowed recipient address must be located in a file unless if it is the
  # same as the domain.  This prevents HTTP_REFERER forging
  if ( open(ALLOWED, "allowed_recipients.txt") ) {
    while(<ALLOWED>) {
      $_ =~ s/[^(\w\-\@\.)]//g;
      if ($_ =~ /^(([^\-][\w\-]*?\.?){1,})[^-](\@{1})([^\-][\w+\.\-])[^\2]{1,}[^\-]$/) {
        push(@allow_mail_to, $_);
      }
    }
  }
